...
*/
    ...
    def parseBinaryStream(inStream: InputStream, pWriter: PrintWriter) {
        ultimately{
            ...
            initCountDownLatch()
            val READ_BUFF_MAX = 60*1000
            var lineCount = 0
            var loopFlag = true
            val inBuff: Array[Byte] = new Array(READ_BUFF_MAX)
            while(loopFlag) {
                ...
                if(ret < 0) {
                    ...
                    @tailrec
                    def parseBytes(buff: Array[Byte]) {
                        if(buff.length == 0) {
                            ;   /* End of recursive */
                        } else if(buff.length < RECORD_SIZE) {
                            println("No enough bytes in last line.")
                            println(s"remain : " +
                                    buff.map(_.toByte).grouped(2).map(a => f"${a(0)}%02X${a(1)}%02X").mkString(","))
                        } else {
                            lineCount += 1
                            startBinaryParserFuture(lineCount, buff.take(RECORD_SIZE))
                            if((lineCount % EXEC_UNIT) == 0) {
                                countDownLatch.await
                                lineCount = 0
                                initCountDownLatch()
                            }
                            parseBytes(buff.drop(RECORD_SIZE))
                        }
                    }
                    parseBytes(if(ret < READ_BUFF_MAX) inBuff.take(ret) else inBuff)
                }
            }
